...
create_matrix <- function(df, row, centre, column){
  ...
  for (replicate_uniq in replicates.test){
    tmp <- test %>% dplyr::filter(!!as.symbol(column) == replicate_uniq)
...
# Set up Directories
cwd <- getwd()
OUTPUT_DIR <- paste0(cwd, "/output_files_FINAL_SQ_combined/")
...
MERGED_SQ_COUNTS_URL <- "https://raw.githubusercontent.com/christopher-holt/bpahangi_host_sex_effect/refs/heads/main/input_files/merged_SQ.counts"
GROUPS_URL <- "https://raw.githubusercontent.com/christopher-holt/bpahangi_host_sex_effect/refs/heads/main/input_files/groups_merged_sq.csv"
...
## Read in Files
```{r}
merged_SQ_counts.orig <- readr::read_delim(MERGED_SQ_COUNTS_URL,
                          "\t", escape_double = FALSE, col_names = FALSE,
                          trim_ws = TRUE, show_col_types = F)


counts.orig <- rbind(counts.orig, merged_SQ_counts.orig)
...
```{r}
b_pahangi_mRNAs <- b_pahangi_annotation %>% dplyr::filter(X3 == "mRNA")
...
## Remove SQ Microfilariae and select only IP samples and merged SQ samples
```{r}
## Remove all microfilariae samples
groups <- groups %>% filter((Infection == "IP" & WormSex != "mf") | Sample %in% c("M_SQ_Bp-males", "M_SQ_Bp-females", "F_SQ_Bp-females", "F_SQ_Bp-males"))

groups <- groups %>% dplyr::filter(!(grepl("SQ_Bp-mf", groups$Sample)))
...
pdf(paste0(FIGURE_DIR, "/dendrogram_merged_SQ.pdf"),
    width = 20,
    height = 10)
  plot(tpm_dendrogram)
invisible(dev.off())
...
pdf(paste0(FIGURE_DIR, "/WGCNA_merged_SQ.pdf"),
    width = 20,
    height = 10)
heatmap.3(na.omit(heatmap_matrix),
              col=hmcol,
              trace="none",
              labRow = NA,
              labCol = F,
...
```{r}
num_reads <- as.data.frame(colSums(counts)) %>% arrange(desc(colSums(counts)))
num_reads <- num_reads %>% rownames_to_column('Sample_Name')
num_reads$Sample_Name <- as.factor(num_reads$Sample_Name)
ggplot(num_reads, aes(x = reorder(Sample_Name, -colSums(counts)), y = colSums(counts))) + 
...
heatmap_matrix <- as.matrix(zscore.log2tpm.de)
#col_fun = circlize::colorRamp2(c(0, 7, 14), c("white", "firebrick1", "firebrick3"))
#col_fun = circlize::colorRamp2(c(-3, 0, 3), c("navyblue", "white", "firebrick3"))
...
hmcol.log <- colorRampPalette(c("navyblue","white","firebrick3"))(12)
